...
@bot.tree.command(name="askwizard", description="Ask a crazed wizard a question")
...
async def ask_gpt(interaction: discord.Interaction, question: str):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Answer All Questions Like A Crazy,very very Horny Wizard."},
...
@bot.tree.command(name="askmom", description="Ask a mother a question (She loves you)")
@app_commands.describe(question="Your question to GPT")
async def ask_gpt(interaction: discord.Interaction, question: str):
    """Command to ask GPT a question"""
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Answer All Questions Like A mother who loves you very much, and appreciates you, and always calls you a good boy."},
                {"role": "user", "content": question}
            ],
            max_tokens=150
        )
...
@bot.tree.command(name="askeboy", description="Ask a uwu e-boy a question")
@app_commands.describe(question="Your question to GPT")
async def ask_gpt(interaction: discord.Interaction, question: str):
    """Command to ask GPT a question"""
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Answer All Questions Like an uwu e-boy that's in love with the user."},
                {"role": "user", "content": question}
            ],
            max_tokens=150
        )
...
@bot.tree.command(name="randchar", description="Generate a random D&D character")
@app_commands.describe(level="Level of the character")
async def randchar(interaction: discord.Interaction, level: int):
    """Generate a random D&D character using OpenAI"""
    await interaction.response.defer()  # Defer the response to allow more processing time
    try:
        ...
        for i in range(6):
            ...
            stats_results.append(total)
        stats_str = ', '.join(str(total) for total in stats_results)
        
        # Prepare the prompt for OpenAI
        prompt = (f"Generate a level {level} D&D character with the following attributes:\n"
                  f"Stats: {stats_str}\n"
                  "Include a class, subclass, backstory, background, ASIs, and subclass abilities up to the given level. "
                  "Also, provide the spell levels available and the number of spells at each level. "
                  "No need for individual spells or personality traits.")

        # Call OpenAI to generate the character
        response = openai.ChatCompletion.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a helpful assistant knowledgeable in D&D character creation."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=2000  # Increase max tokens to get a detailed response
        )
        character_description = response.choices[0].message['content'].strip()

        # Ensure the response does not exceed 2000 characters
        if len(character_description) > 2000:
            character_description = character_description[:2000]

        # Extract the name, stats, summary, ASIs, subclass abilities, and spell information
        lines = character_description.split('\n')
        character_name = lines[0].strip() if len(lines) > 0 else "Unknown Name"
        summary_index = 1
        while summary_index < len(lines) and not lines[summary_index].strip():
            summary_index += 1
        summary_end_index = summary_index + 3
        summary = '\n'.join(lines[summary_index:summary_end_index]).strip()
        detailed_info_index = summary_end_index
        while detailed_info_index < len(lines) and not lines[detailed_info_index].strip():
            detailed_info_index += 1
        detailed_info = '\n'.join(lines[detailed_info_index:]).strip()

        # Split the detailed info to separate spell levels
        spell_section_index = detailed_info.lower().find("spell levels available:")
        if spell_section_index != -1:
            spells_info = detailed_info[spell_section_index + len("spell levels available:"):].strip()
            detailed_info = detailed_info[:spell_section_index].strip()
        else:
            spells_info = ""

        # Format spell levels into a table
        spell_lines = spells_info.split('\n')
        spell_table = "```"
        ...
        summary_message = f"**Character Name**: {character_name}\n**Stats**: {stats_str}\n**Summary**:\n{summary}"
        detailed_info_message = f"**Detailed Information for {character_name}**:\n{detailed_info}\n\n**Spell Levels Available**:\n{spell_table}"

        # Send the summary to the interaction channel
        await interaction.followup.send(summary_message[:2000])

        # Send the detailed information and spell levels via DM if it exceeds the limit
        user = interaction.user
        if len(detailed_info_message) > 2000:
            detailed_info_chunks = [detailed_info_message[i:i+2000] for i in range(0, len(detailed_info_message), 2000)]
            for chunk in detailed_info_chunks:
                await user.send(chunk)
        else:
            await user.send(detailed_info_message)
    except Exception as e:
...
@bot.tree.command(name="kill", description="Kill the bot")
async def reboot(interaction: discord.Interaction):
    """Command to reboot the bot"""
    await interaction.response.send_message("Killing the bot...")
    await bot.close()
    subprocess.Popen(["C:\\Users\\Jacob\\Desktop\\restart_bot.bat"])
    sys.exit()
